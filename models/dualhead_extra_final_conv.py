"""Dual-head CNN variant with an extra convolution in each branch."""
...
class DualHeadExtraFinalConvCNN(nn.Module):
    def __init__(
        self,
        input_length: int,
        num_classes: int,
        ...
        self.medium_conv = nn.Sequential(
            ...
            nn.Conv1d(embed_dim, embed_dim, 11, padding=5), nn.BatchNorm1d(embed_dim), nn.ReLU(),
        )
        self.local_conv = nn.Sequential(
            ...
            nn.Conv1d(embed_dim, embed_dim, 7, padding=3), nn.BatchNorm1d(embed_dim), nn.ReLU(),
        )
