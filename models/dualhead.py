"""Baseline dual-head CNN for one-dimensional IR spectra."""
...
class DualHeadCNN(nn.Module):
    def __init__(
        ...
        self.medium_conv = nn.Sequential(
            nn.Conv1d(1, 32, 71, padding=35),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.MaxPool1d(2),
            nn.Conv1d(32, 64, 61, padding=30),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.MaxPool1d(2),
            nn.Conv1d(64, embed_dim, 25, padding=12),
            nn.BatchNorm1d(embed_dim),
            nn.ReLU(),
        )
        self.local_conv = nn.Sequential(
            nn.Conv1d(1, 32, 25, padding=12),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.MaxPool1d(2),
            nn.Conv1d(32, 64, 17, padding=8),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.MaxPool1d(2),
            nn.Conv1d(64, embed_dim, 11, padding=5),
            nn.BatchNorm1d(embed_dim),
            nn.ReLU(),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attended = torch.bmm(attention, value).permute(0, 2, 1)
        attended = self.dropout(attended)
        fused = self.fuse_conv(torch.cat([medium, attended], dim=1))
